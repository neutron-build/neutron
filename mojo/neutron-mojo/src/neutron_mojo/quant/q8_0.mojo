...
def q8_0_block_size() -> Int:
    """Return the Q8_0 block size (32)."""
    return 32


def q8_0_bytes_per_block() -> Int:
...
def quantize_q8_0(value: Float32, scale: Float32) -> Int8:
...
def dequantize_q8_0(value: Int8, scale: Float32) -> Float32:
...
def quantize_q8_0_block(
    input: Pointer[Float32, ...],
    output: Pointer[mut=True, Int8, ...],
    block_size: Int,
) -> Float32:
...
def dequantize_q8_0_block(
    ...
    for i in range(block_size):
        var q = input.load(i)
        output.unsafe_store(i, dequantize_q8_0(q, scale))
...
def calc_q8_0_buffer_size(num_elements: Int) -> Int:
