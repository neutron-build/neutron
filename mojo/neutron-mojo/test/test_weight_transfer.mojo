...
def assert_close(a: Float32, b: Float32, atol: Float64 = 1e-5) raises:
...
def _make_tiny_params() -> ModelParams:
...
def test_weight_mapping_struct() raises:
...
def test_weight_mapping_copy() raises:
...
def test_build_mapping_count() raises:
...
def test_build_mapping_sources() raises:
...
def test_model_to_tape_embed() raises:
    ...
    for i in range(p.vocab_size * p.hidden_dim):
        assert_close(tape.get_data(embed_idx, i), Float32(0.01 * Float64(i + 1)))
    print("  model_to_tape_embed: PASS")


def test_model_to_tape_final_norm() raises:
    """model_to_tape correctly copies final_norm weights."""
    var p = _make_tiny_params()
    var model = Model(p)
    for i in range(p.hidden_dim):
        model.final_norm.set(i, Float32(0.5 + 0.1 * Float64(i)))
    ...
    for i in range(p.hidden_dim):
        assert_close(tape.get_data(norm_idx, i), Float32(0.5 + 0.1 * Float64(i)))
    print("  model_to_tape_final_norm: PASS")


def test_model_to_tape_lm_head() raises:
    """model_to_tape correctly copies lm_head weights."""
    var p = _make_tiny_params()
    var model = Model(p)
    for i in range(p.vocab_size * p.hidden_dim):
        model.lm_head.set(i, Float32(0.02 * Float64(i + 1)))
    ...
    for i in range(p.vocab_size * p.hidden_dim):
        assert_close(tape.get_data(head_idx, i), Float32(0.02 * Float64(i + 1)))
    print("  model_to_tape_lm_head: PASS")


def test_model_to_tape_layer_weights() raises:
    ...
    for i in range(total):
        model.layer_weights.set(i, Float32(0.001 * Float64(i + 1)))
...
def test_tape_to_model_creates_model() raises:
...
def test_roundtrip_embed() raises:
    """Roundtrip: Model -> Tape -> Model preserves embed weights."""
    var p = _make_tiny_params()
    var model = Model(p)
    for i in range(p.vocab_size * p.hidden_dim):
        model.embed.set(i, Float32(0.03 * Float64(i + 1)))
    ...
    for i in range(p.vocab_size * p.hidden_dim):
        assert_close(model2.embed.get(i // p.hidden_dim, i % p.hidden_dim), Float32(0.03 * Float64(i + 1)))
...
def test_roundtrip_full() raises:
    ...
    for i in range(p.vocab_size * p.hidden_dim):
        ...
        model.lm_head.set(i, Float32(0.02 * Float64(i + 1)))
    for i in range(p.hidden_dim):
        model.final_norm.set(i, Float32(1.0 + 0.1 * Float64(i)))
...
def test_roundtrip_layer1() raises:
    ...
    var total_lw = p.num_layers * p.layer_weight_count()
    for i in range(total_lw):
        model.layer_weights.set(i, Float32(0.005 * Float64(i + 1)))
