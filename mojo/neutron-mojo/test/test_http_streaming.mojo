...
def test_sse_event_format() raises:
...
def test_sse_done_format() raises:
...
def test_sse_escape_special_chars() raises:
...
def test_sse_multi_token_stream() raises:
...
def test_parse_streaming_request() raises:
...
def test_non_streaming_still_works() raises:
...
def test_token_event_to_sse() raises:
