#!/usr/bin/env python3
"""
Benchmark: vLLM Llama-3-8B Inference
======================================
Measures vLLM serving performance for Llama-3-8B at various concurrency levels.
This script starts a vLLM server as a subprocess, sends concurrent requests via
...
Setup:
    ...
    2. Make sure you have access to the Llama-3-8B model. Either:
       a. Download from HuggingFace (requires approval):
          huggingface-cli login
          huggingface-cli download meta-llama/Meta-Llama-3-8B-Instruct
       b. Or use a local path / alternative model ID.

    3. Ensure sufficient GPU memory (~16GB for FP16, ~8GB for AWQ/GPTQ).
...
Configuration:
    ...
    --port              Port for vLLM server (default: 8400)
...
# Server management
# ---------------------------------------------------------------------------
def start_vllm_server(model, port, tp_size, timeout):
    """Start a vLLM server subprocess and wait until it's ready."""
    cmd = [
    ...
    print(f"  python -m vllm.entrypoints.openai.api_server --model {model} --port {port}")
    sys.exit(1)
            ...
            for line in resp.iter_lines():
                ...
                if line.startswith("data: "):
                    ...
                    try:
                        ...
                        if text:
                            output_tokens += 1  # Approximate: each SSE chunk ~ 1 token
...
def main():
    parser = argparse.ArgumentParser(description="vLLM Llama-3-8B Benchmark")
    ...
    try:
        if not args.no_server:
            try:
                ...
                print("ERROR: vLLM is required. Install with:")
        ...
        for conc in args.concurrency:
            ...
            print(f"BENCHMARK: vllm_llama3 | concurrency: {conc} | "
