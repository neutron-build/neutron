...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let router = Router::new()
        .state(store)
        // Middleware order follows FRAMEWORK_CONTRACT.md:
        // RequestID → Logging → Recovery → CORS → Compression → RateLimit → Auth → Timeout → OTel.
        // RequestID must precede Logging so every log line carries the request id.
        // (P1.4 will replace this hand-wired stack with `default_stack()`, which
