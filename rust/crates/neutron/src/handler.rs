...
// `HandlerWrapper` auto-derives `Send`/`Sync` from its fields: `handler: H`
// contributes `H: Send`/`Sync`, and `PhantomData<fn() -> T>` is unconditionally
// `Send + Sync` for all `T`. The previous hand-written `unsafe impl`s asserted
// exactly what the compiler already guarantees, so they were dead `unsafe` —
// removed. The compile-time assertion below fails to build if the bound ever
// regresses (e.g. a non-`Send` field is added).
const _: fn() = || {
    fn assert_send_sync<T: Send + Sync>() {}
    assert_send_sync::<HandlerWrapper<fn(), ()>>();
};
