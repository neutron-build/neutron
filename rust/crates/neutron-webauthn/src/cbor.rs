...
fn peek(data: &[u8], pos: usize) -> Result<u8, WebAuthnError> {
    data.get(pos)
        .copied()
        .ok_or_else(|| WebAuthnError::Cbor("unexpected end of data".into()))
}
...
fn read_int(data: &[u8], pos: &mut usize) -> Result<i64, WebAuthnError> {
    ...
    match major {
        ...
        1 => {
            // negative integer: -1 - v
            let (v, advance) = decode_additional(data, *pos)?;
            *pos += advance;
            Ok(-1 - v as i64)
        }
        _ => Err(WebAuthnError::Cbor(format!(
            "expected integer (major 0 or 1), got major {major}"
        ))),
    }
}
...
fn read_bytes(data: &[u8], pos: &mut usize) -> Result<Vec<u8>, WebAuthnError> {
    ...
    if major != 2 {
        return Err(WebAuthnError::Cbor(format!(
            "expected bytes (major 2), got major {major}"
        )));
    }
    let (len, advance) = decode_additional(data, *pos)?;
    *pos += advance;
...
fn decode_additional(data: &[u8], pos: usize) -> Result<(u64, usize), WebAuthnError> {
    ...
    match info {
        ...
        24 => {
            let v = *data
                .get(pos + 1)
                .ok_or_else(|| WebAuthnError::Cbor("truncated".into()))? as u64;
            Ok((v, 2))
        }
        25 => {
            let hi = *data
...
mod tests {
    ...
    fn build_cose_key(x: &[u8; 32], y: &[u8; 32]) -> Vec<u8> {
        ...
        v.push(0x03);
        v.push(0x26); // 0x26 = major 1, additional 6
