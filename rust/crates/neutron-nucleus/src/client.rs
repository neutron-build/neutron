...
use crate::models::{
    blob::BlobModel, cdc::CdcModel, columnar::ColumnarModel, datalog::DatalogModel,
    document::DocumentModel, fts::FtsModel, geo::GeoModel, graph::GraphModel, kv::KvModel,
    pubsub::PubSubModel, sql::SqlModel, streams::StreamModel, timeseries::TimeSeriesModel,
    vector::VectorModel,
};
...
fn nucleus_version_re() -> &'static Regex {
    ...
    RE.get_or_init(|| Regex::new(r"Nucleus\s+(\d+\.\d+\.\d+)").unwrap())
}
