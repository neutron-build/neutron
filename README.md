...
- **14 data models, one database.** Nucleus stands in for Postgres + Redis + a vector store + a search index + a graph DB: SQL, KV, Vector, TimeSeries, Document, Graph, FTS, Geo, Blob, Streams, Columnar, Datalog, CDC, and PubSub — all over the Postgres wire protocol, so any Postgres client connects. 346,767 lines of Rust, 5,246 declared tests, MVCC + WAL crash recovery.
