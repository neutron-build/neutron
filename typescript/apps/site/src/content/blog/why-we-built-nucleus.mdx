...
Every data model has a Write-Ahead Log. Before any write is applied to the primary data structure, it's written to the WAL. If the process crashes mid-write, the WAL lets us replay and recover to a consistent state on restart.

This was a significant engineering effort — each model has different write patterns and different recovery semantics. The vector index (HNSW graph) has very different WAL needs than the timeseries store (append-only deltas). We shipped WAL for all fourteen models in Phase 1.
