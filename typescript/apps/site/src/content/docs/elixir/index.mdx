...
> **Status: the Nucleus client cannot connect yet.** The framework half — Plug
> routing, Bandit, supervisors, channels — works. Anything needing a database
> connection does not, including every example on the
> [Nucleus Client](/docs/elixir/nucleus) page.
>
> Postgrex runs a type-bootstrap query on connect that Nucleus cannot yet answer.
> Two of the three causes are fixed (`pg_range` and
> `pg_type.typsend`/`typreceive`/`typoutput` were added to the catalog); the
> remaining one is a real SQL gap — `ARRAY (SELECT ...)` in a function argument
> returns `ERROR 0A000 subquery in function args`. The pool retries, so the
> caller sees a queue timeout that never names the cause.
>
> This affects **every Elixir, Ecto and Phoenix application**, not a subset of
> features. Verified against the engine 2026-08-15.
