...
> **Status: connects, with gaps in the model coverage.** Postgrex could not
> complete a connection to Nucleus at all until 2026-08-15 — its type bootstrap
> needed `ARRAY (SELECT ...)`, which the engine rejected, so every Elixir, Ecto
> and Phoenix application timed out at connect with no cause named. That is
> fixed, and Elixir now runs in the live conformance suite: 32 of 42 cases pass,
> none fail.
>
> The remaining 7 are model methods this SDK does not implement yet — see
> [Nucleus Client](/docs/elixir/nucleus) for the list. Verified against a running
> engine 2026-08-15.
