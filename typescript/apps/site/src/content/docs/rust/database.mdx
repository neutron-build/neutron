...
> runs python, go, typescript and elixir against a running engine; the Rust
> client is not in it. The shapes below come from the SDK source and its own unit tests, not
