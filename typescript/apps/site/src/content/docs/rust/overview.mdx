...
description: Modular Rust services, workers, and application backends with Hyper, Tokio, and Nucleus.
---

Neutron for Rust is a modular framework for asynchronous services, background workers, and application backends. Its HTTP core uses Hyper and Tokio, with typed extractors and composable middleware. Optional crates add jobs, database access, storage, and telemetry; Neutron Desktop also uses Rust for its backend.

Use the modules that fit your application alongside the wider Rust ecosystem. Neutron’s current core focuses on networked applications. Choosing Rust gives you control over resources and concurrency; it does not establish that a particular application is faster without measurement.
