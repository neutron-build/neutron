...
The SDK must read `NEUTRON_HOST` and `NEUTRON_PORT` for an assigned port to work. Declare `ports` explicitly where an SDK version does not yet do so.
...
A runnable two-language example is in [`cli/examples/application`](https://github.com/neutron-build/neutron/tree/main/cli/examples/application).
