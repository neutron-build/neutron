...
-- Remove a member
SELECT KV_SREM('tags:post:1', 'rust');

-- Cardinality
SELECT KV_SCARD('tags:post:1');
```

Set-to-set operations (`SINTER`/`SUNION`/`SDIFF`) and TTL removal (`PERSIST`)
are not implemented. Intersect two tag sets in SQL against `KV_SMEMBERS` until
they are.
...
-- Count members
SELECT KV_ZCARD('leaderboard');

-- Remove a member
SELECT KV_ZREM('leaderboard', 'player2');
```
