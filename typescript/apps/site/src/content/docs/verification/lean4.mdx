...
The suite compiles with **zero `sorry`**, but it declares **25 explicit `axiom`s** — facts assumed rather than derived from Lean's core. Being explicit about them is the point: what is assumed is stated in the open and can be audited. They fall into three groups:
...
All 26 files compile with **0 `sorry`** across **92 theorems**, and `scripts/axioms.sh` checks what those proofs rest on: nothing beyond Lean's own axioms and **3 stated assumptions about an opaque SHA-256**. The structural axioms were discharged on 2026-08-17. The proofs cover Lean models of the algorithms listed above; they do not prove properties of the production Rust directly, and that remains the open thread — moving from hand-written models toward Aeneas-generated ones.
