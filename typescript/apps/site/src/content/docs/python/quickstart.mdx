...
pip install "neutron-py[all]"  # every extra
```

The full set is `ai`, `crypto`, `granian`, `rich`, `all` — plus `test` for
development. There is no `redis` or `otel` extra; this block advertised both
until 2026-08-17.
...
The generated `pyproject.toml` installs `neutron-py` from the Neutron
repository (`git+https://github.com/neutron-build/neutron.git#subdirectory=python`).
`neutron dev` runs `python -m neutron dev app.main:app` with the project's
`.venv`, on `NEUTRON_PORT` (default 8000).
