...
async fn wal_persistence_all_models() {
    ...
    // Phase 1: Write data
    {
        ...
        // SQL
        db.execute("CREATE TABLE wal_test (id INT, name TEXT)")
            .await
            .unwrap();
        db.execute("INSERT INTO wal_test VALUES (1, 'alice')")
            .await
            .unwrap();

        // KV
        db.kv()
            .set("test_key", Value::Text("test_val".into()), None);
        ...
        // Blob
        db.blob()
            .put("blob_key", b"blob_data_test", Some("text/plain"));
        ...
        // TimeSeries
        db.execute("SELECT TS_INSERT('metric1', 1000, 42.0)")
            .await
            .unwrap();

        // Vector
        db.execute("CREATE TABLE vec_wal (id INT, v VECTOR(3))")
            .await
            .unwrap();
        db.execute("INSERT INTO vec_wal VALUES (1, VECTOR('[1.0,2.0,3.0]'))")
            .await
            .unwrap();

        // Columnar
        db.execute("SELECT COLUMNAR_INSERT('col_wal', 'metric', '99')")
            .await
            .unwrap();

        // Datalog
        db.execute("SELECT DATALOG_ASSERT('parent(alice, bob)')")
            .await
            .unwrap();

        // Streams
        db.execute("SELECT STREAM_XADD('wal_stream', 'key', 'val_1')")
            .await
            .unwrap();
    ...
    // Phase 2: Reopen and verify
    {
        ...
        // Read the count through a temporary guard so it is released at the end
        // of this statement, well before the next `.await` below.
        let graph_ok = graph_handle.read().node_count() > 0;
