...
async fn fts_rank_is_bm25_saturating_in_tf() {
    ...
    assert!(
        s1 > 0.0 && s2 > s1 && s3 > s2,
        "FTS_RANK must increase with tf: {s1} {s2} {s3}"
    );
    ...
    assert!(
        s3 - s2 < s2 - s1,
        "FTS_RANK must saturate (concave in tf): {s1} {s2} {s3}"
    );

    // A term absent from the doc contributes nothing.
    assert_eq!(
        score(&ex, "alpha beta", "zzz").await,
        0.0,
    ...
    assert!(
        many_long > few_short,
        "more occurrences must rank >= fewer: {many_long} vs {few_short}"
    );
}
