...
async fn projection_between_literals_not_reused() {
    ...
    ex.execute_with_session(s, "CREATE TABLE t (id INTEGER PRIMARY KEY, c6 INTEGER)")
        .await
        .unwrap();
    ex.execute_with_session(s, "INSERT INTO t (id,c6) VALUES (1,11)")
        .await
        .unwrap();
    ...
    assert_eq!(
        second,
        vec![vec!["Bool(true)".to_string()]],
        "second query reused first's BETWEEN bounds"
    );
    ...
    assert_eq!(
        nd,
        vec![vec!["Bool(true)".to_string()]],
        "non-distinct projection reused stale bounds"
    );
}
...
async fn projection_string_literal_not_reused() {
    ...
    let s = ex.create_session();
    ex.execute_with_session(s, "CREATE TABLE t (id INTEGER PRIMARY KEY, c TEXT)")
        .await
        .unwrap();
    ex.execute_with_session(s, "INSERT INTO t (id,c) VALUES (1,'blue')")
        .await
        .unwrap();
    ...
    assert_eq!(
        second,
        vec![vec!["Bool(true)".to_string()]],
        "string-literal projection reused stale value"
    );
}
...
async fn where_point_lookup_reuse_still_correct() {
    ...
    let s = ex.create_session();
    ex.execute_with_session(s, "CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)")
        .await
        .unwrap();
    ex.execute_with_session(s, "INSERT INTO t (id,v) VALUES (1,10),(2,20),(3,30)")
        .await
        .unwrap();
    ...
    assert_eq!(
        r2,
        vec![vec!["Int32(20)".to_string()]],
        "WHERE point-lookup reuse returned wrong row"
    );
