...
impl Schema {
    ...
    fn nonid<'a>(&'a self, rng: &mut Rng) -> &'a Col {
        &self.cols[1 + rng.below(self.cols.len() - 1)]
    }
}
...
fn main_impl() {
    ...
    'outer: for iter in 0..iterations {
            ...
            let db = match open(&tmp.0) {
                ...
                Err(_) => {
                    ...
                    if findings <= max_report {
                        println!("─── REOPEN PANIC (initial open, iter {iter})\n");
                    }
                    continue 'outer;
                }
            };
            ...
            for _ in 0..ops_per {
                ...
                match rng.below(5) {
                    0 | 1 | 2 => {
                        let id = next_id;
                        next_id += 1;
                        let _ = exec(&db, &gen_insert(&schema, &mut rng, id));
                    }
                    3 => {
                        let c = schema.nonid(&mut rng).clone();
                        let _ = exec(
                            &db,
                            &format!(
                    ...
                    _ => {
                        let _ = exec(
                            &db,
                            &format!("DELETE FROM t WHERE id = {}", rng.int(1, next_id.max(1))),
                        );
                    }
                }
        ...
        let original = match std::fs::read(tmp.wal()) {
            Ok(b) => b,
        ...
        for _ in 0..variants {
            ...
            if std::fs::write(tmp.wal(), &torn).is_err() {
                continue;
            }
            ...
            let db = match open(&tmp.0) {
                ...
                Err(_) => {
                    ...
                    if findings <= max_report {
                        println!(
                            "─── REOPEN PANIC (torn WAL, iter {iter}) ── tear={:?}",
                            tear
                        );
            ...
            let recovered = match snapshot(&db, &cols) {
                ...
                Snap::HardError => {
                    ...
                    if findings <= max_report {
                        println!(
                            "─── RECOVERY READ FAILED (torn WAL, iter {iter}) ── tear={:?}\n",
                            tear
                        );
                    }
                    drop(db);
            ...
            if recovered.len() < final_rows {
                cov_partial += 1;
            }

            let mut bad: Option<(String, Vec<String>)> = None;
            for (id, row) in &recovered {
                if !ever.contains(row) {
                    bad = Some((id.clone(), row.clone()));
                    break;
                }
            }
            ...
            if let Some((id, got)) = bad {
                ...
                if findings <= max_report {
                    ...
                    println!(
                        "  recovered={} rows, ever-committed tuples={}\n",
                        recovered.len(),
                        ever.len()
                    );
                }
