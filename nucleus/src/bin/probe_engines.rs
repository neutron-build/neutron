...
impl Schema {
    ...
    fn of<F: Fn(&Col) -> bool>(&self, f: F) -> Vec<&Col> { self.cols.iter().filter(|c| f(c)).collect() }
    fn int_cols(&self) -> Vec<&Col> { self.of(|c| c.ty == Ty::Int) }
    fn nn_nonid(&self) -> Vec<&Col> { self.of(|c| c.nn && c.name != "id") }
...
fn gen_inserts(s: &Schema, rng: &mut Rng, rows: usize) -> String {
    ...
    let vals: Vec<String> = (1..=rows).map(|id| {
...
fn gen_predicate(s: &Schema, rng: &mut Rng, depth: u32) -> String {
    ...
    if rng.chance(12) { return format!("NOT ({})", gen_predicate(s, rng, 0)); }
    let c = s.pick(rng);
    match rng.below(6) {
        ...
        2 => {
            ...
            format!("{} {}IN ({})", c.name, if rng.chance(40) { "NOT " } else { "" }, items.join(","))
        }
...
fn gen_orderby(s: &Schema, rng: &mut Rng) -> String {
    ...
    if !nn.is_empty() && rng.chance(70) {
        let c = *rng.pick(&nn);
        keys.push(format!("{} {}", c.name, if rng.chance(50) { "ASC" } else { "DESC" }));
    }
...
fn gen_query(s: &Schema, rng: &mut Rng, rows: usize) -> (String, bool) {
    match rng.below(6) {
        0 => {
            ...
            let w = if rng.chance(55) { format!(" WHERE {}", gen_predicate(s, rng, 2)) } else { String::new() };
            (format!("SELECT {} FROM t{w}", aggs.join(", ")), true)
        }
        1 => {
            let gcols: Vec<&Col> = s.nn_nonid().into_iter().filter(|c| c.ty != Ty::Real).collect();
            let g = if gcols.is_empty() { "id" } else { rng.pick(&gcols).name };
            let agg = gen_agg(s, rng);
            let w = if rng.chance(45) { format!(" WHERE {}", gen_predicate(s, rng, 1)) } else { String::new() };
            (format!("SELECT {g}, {agg} FROM t{w} GROUP BY {g} ORDER BY {g} ASC"), true)
        }
        2 => {
            let proj = if rng.chance(30) { "*".to_string() } else {
                let cols: Vec<&str> = s.cols.iter().map(|c| c.name).collect();
                (0..1 + rng.below(3)).map(|_| *rng.pick(&cols)).collect::<Vec<_>>().join(", ")
            };
            (format!("SELECT DISTINCT {proj} FROM t WHERE {}", gen_predicate(s, rng, 1)), false)
        }
        _ => {
            let cols: Vec<&str> = s.cols.iter().map(|c| c.name).collect();
            let proj = if rng.chance(25) { "*".to_string() } else {
                (0..1 + rng.below(3)).map(|_| *rng.pick(&cols)).collect::<Vec<_>>().join(", ")
            };
            let w = if rng.chance(60) { format!(" WHERE {}", gen_predicate(s, rng, 2)) } else { String::new() };
            let order = format!(" {}", gen_orderby(s, rng));
            let limit = if rng.chance(40) { format!(" LIMIT {}", rng.int(1, rows as i64)) } else { String::new() };
...
fn gen_mutation(s: &Schema, rng: &mut Rng, next_id: &mut i64) -> String {
    match rng.below(3) {
        0 => {
            let names: Vec<&str> = s.cols.iter().map(|c| c.name).collect();
            let cells: Vec<String> = s.cols.iter().enumerate()
                .map(|(i, c)| if i == 0 { next_id.to_string() } else { gen_value(rng, c) }).collect();
            *next_id += 1;
            format!("INSERT INTO t ({}) VALUES ({})", names.join(","), cells.join(","))
        }
        1 => {
            let c = *rng.pick(&s.cols.iter().skip(1).collect::<Vec<_>>());
            format!("UPDATE t SET {} = {} WHERE {}", c.name, gen_value(rng, c), gen_predicate(s, rng, 1))
        }
...
fn run_pair(engine_b: &str, seed: u64, iterations: usize, queries_per: usize, max_report: usize) -> usize {
    ...
    'outer: for iter in 0..iterations {
        ...
        let exb = Arc::new(Executor::new(Arc::new(Catalog::new()), make_engine(engine_b)));
        let mut ops = vec![ddl.clone(), inserts.clone()];
        for stmt in [&ddl, &inserts] {
            if !exec(&exa, stmt) || !exec(&exb, stmt) { continue 'outer; }
        }
        ...
        for _ in 0..queries_per {
            ...
            if rng.chance(30) {
                ...
                if ra != rb { continue 'outer; } // mutation acceptance differs → desync, skip
                if ra { ops.push(m); }
                continue;
            }
                ...
                (Ok(a), Ok(b)) => {
                    if !compare(a.clone(), b.clone(), ordered) {
                        ...
                        if divergences <= max_report {
                            ...
                            if !ordered { av.sort(); bv.sort(); }
                            println!("─── DIVERGENCE #{divergences} (Mvcc vs {engine_b}, iter {iter}, seed {seed}) ───");
                            println!("  query : {q}");
                            println!("  mvcc ({} rows)   : {:?}", av.len(), &av[..av.len().min(6)]);
                            println!("  {engine_b} ({} rows): {:?}", bv.len(), &bv[..bv.len().min(6)]);
...
fn main_impl() {
    ...
    let mut engines = vec!["lsm".to_string(), "memory".to_string(), "columnar".to_string()];
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--engine" => { i += 1; engines = vec![args[i].clone()]; }
            _ => {}
        }
        i += 1;
    }
    std::panic::set_hook(Box::new(|_| {}));
    println!("Nucleus engine-vs-engine differential fuzzer\nseed={seed} iterations={iterations} queries/iter={queries_per}\n");
    ...
    if total_div == 0 { println!("\nAll engines agree with Mvcc. 🎯"); } else { std::process::exit(1); }
}
