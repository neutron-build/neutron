...
async fn main() {
    ...
    match pg_load {
        Some(pg) => println!(
            "  Bulk insert {} rows: Nucleus {}ms  PG {}ms",
            cfg.rows, nucleus_load, pg
        ),
        None => println!(
            "  Bulk insert {} rows: Nucleus {}ms",
            cfg.rows, nucleus_load
        ),
    }
