...
use nucleus::binary_wire::decoder::{message_types, DecodeError, DecodedFrame, Decoder};
...
const PARSER_NAMES: &[&str] = &[
    "query", "prepared_stmt", "bind", "execute", "command_complete",
    "data_row", "error", "handshake", "authentication", "ready",
    "column_metadata", "result_end", "begin_txn", "commit_txn",
    "rollback_txn", "parameter_status",
];
...
fn run_parser(name: &str, payload: &[u8]) {
    match name {
        "query" => { let _ = Decoder::parse_query(payload); }
        "prepared_stmt" => { let _ = Decoder::parse_prepared_stmt(payload); }
        "bind" => { let _ = Decoder::parse_bind(payload); }
        "execute" => { let _ = Decoder::parse_execute(payload); }
        "command_complete" => { let _ = Decoder::parse_command_complete(payload); }
        "data_row" => { let _ = Decoder::parse_data_row(payload); }
        "error" => { let _ = Decoder::parse_error(payload); }
        "handshake" => { let _ = Decoder::parse_handshake(payload); }
        "authentication" => { let _ = Decoder::parse_authentication(payload); }
        "ready" => { let _ = Decoder::parse_ready(payload); }
        "column_metadata" => { let _ = Decoder::parse_column_metadata(payload); }
        "result_end" => { let _ = Decoder::parse_result_end(payload); }
        "begin_txn" => { let _ = Decoder::parse_begin_txn(payload); }
        "commit_txn" => { let _ = Decoder::parse_commit_txn(payload); }
        "rollback_txn" => { let _ = Decoder::parse_rollback_txn(payload); }
        "parameter_status" => { let _ = Decoder::parse_parameter_status(payload); }
        _ => {}
    }
}
...
fn run_handshake(name: &str, payload: &[u8]) {
    match name {
        "auth_challenge" => { let _ = AuthChallenge::decode(payload); }
        "auth_response" => { let _ = AuthResponse::decode(payload); }
        _ => {}
    }
}
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        loop {
            ...
            match dec.parse_frame() {
                Ok(Some(DecodedFrame { message_type, payload })) => {
...
fn dispatch_by_tag(tag: u8, payload: &[u8]) {
    match tag {
        message_types::QUERY => { let _ = Decoder::parse_query(payload); }
        message_types::PREPARED_STMT => { let _ = Decoder::parse_prepared_stmt(payload); }
        message_types::BIND => { let _ = Decoder::parse_bind(payload); }
        message_types::EXECUTE => { let _ = Decoder::parse_execute(payload); }
        message_types::COMMAND_COMPLETE => { let _ = Decoder::parse_command_complete(payload); }
        message_types::DATA_ROW => { let _ = Decoder::parse_data_row(payload); }
        message_types::ERROR => { let _ = Decoder::parse_error(payload); }
        message_types::HANDSHAKE => { let _ = Decoder::parse_handshake(payload); }
        message_types::AUTHENTICATION => { let _ = Decoder::parse_authentication(payload); }
        message_types::READY => { let _ = Decoder::parse_ready(payload); }
        message_types::COLUMN_METADATA => { let _ = Decoder::parse_column_metadata(payload); }
        message_types::RESULT_END => { let _ = Decoder::parse_result_end(payload); }
        message_types::BEGIN_TXN => { let _ = Decoder::parse_begin_txn(payload); }
        message_types::COMMIT_TXN => { let _ = Decoder::parse_commit_txn(payload); }
        message_types::ROLLBACK_TXN => { let _ = Decoder::parse_rollback_txn(payload); }
        message_types::PARAMETER_STATUS => { let _ = Decoder::parse_parameter_status(payload); }
        _ => {}
    }
}
...
fn main_impl() {
    ...
    println!("target: nucleus::binary_wire (Decoder::parse_frame + parse_* + handshake decoders)\n");
    ...
    for _ in 0..iterations {
        ...
        if drive_stream(&r) {
            ...
            findings.push(Finding { kind: "stream(random)", detail: "parse_frame loop panicked".into(), input: m });
        }
        ...
        if drive_stream(&f) {
            ...
            findings.push(Finding { kind: "stream(malformed-frame)", detail: "parse_frame loop panicked".into(), input: m });
        }
        ...
        let k = 1 + rng.below(4);
        for _ in 0..k { multi.extend_from_slice(&gen_malformed_frame(&mut rng)); }
        if drive_stream(&multi) {
            let m = minimize(&multi, drive_stream);
            findings.push(Finding { kind: "stream(multi-frame)", detail: "parse_frame loop panicked".into(), input: m });
        }
        ...
        if drive_payload(&p) {
            ...
            findings.push(Finding { kind: "payload(all-parsers)", detail: "a parse_* fn panicked".into(), input: m });
        }

        if findings.len() >= max_report { break; }
    }
    ...
    if !findings.is_empty() {
        ...
        for (idx, f) in findings.iter().enumerate() {
            ...
            if let Some(m) = msg { println!("   panic msg : {m}"); }
            println!();
        }
    }
...
fn capture_panic_msg(input: &[u8], kind: &str) -> Option<String> {
    ...
    let res = catch_unwind(AssertUnwindSafe(|| {
        match kind {
            "payload(all-parsers)" => {
                for p in PARSER_NAMES { run_parser(p, &input); }
                for h in HANDSHAKE_NAMES { run_handshake(h, &input); }
            }
            _ => {
                let mut dec = Decoder::new();
                dec.feed(&input);
                ...
                loop {
                    ...
                    match dec.parse_frame() {
                        Ok(Some(fr)) => {
                            ...
                            for p in PARSER_NAMES { run_parser(p, &fr.payload); }
                            for h in HANDSHAKE_NAMES { run_handshake(h, &fr.payload); }
                        }
                        _ => break,
                    }
                }
    ...
    match res {
        Ok(()) => None,
        Err(e) => {
            if let Some(s) = e.downcast_ref::<&str>() { Some((*s).to_string()) }
            else if let Some(s) = e.downcast_ref::<String>() { Some(s.clone()) }
            else { Some("<non-string panic>".into()) }
        }
    }
}
