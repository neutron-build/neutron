...
fn open_disk(dir: &PathBuf) -> Result<Database, String> {
    ...
    Database::builder().disk(file).build().map_err(|e| format!("open: {e:?}"))
}
...
fn run_disk_sql(seed: u64, iterations: usize, ops_per: usize, max_report: usize,
                total: &mut usize, divergences: &mut usize) {
    'outer: for iter in 0..iterations {
        ...
        for cycle in 0..cycles {
            let db = match open_disk(&tmp.0) {
                ...
                Err(e) => {
                    *divergences += 1;
                    if *divergences <= max_report { println!("─── [disk-sql] OPEN FAILED #{divergences} (iter {iter}) ── {e}\n", divergences = *divergences); }
                    continue 'outer;
                }
            };
            if cycle == 0 {
                ...
                *total += 1;
                let recovered = match snapshot(&db, &cols) {
                    Ok(s) => s,
                    Err(e) => { *divergences += 1; if *divergences <= max_report { println!("─── [disk-sql] READ FAILED #{} (iter {iter}) ── {e}\n", *divergences); } continue 'outer; }
                };
                if &recovered != exp {
                    *divergences += 1;
                    if *divergences <= max_report {
                        println!("─── [disk-sql] RECOVERY DIVERGENCE #{} (iter {iter}, cycle {cycle}, seed {seed}) ───", *divergences);
                        println!("  schema    : {}", schema.ddl());
                        println!("  pre-down  ({} rows): {:?}", exp.len(), &exp[..exp.len().min(8)]);
                        println!("  recovered ({} rows): {:?}", recovered.len(), &recovered[..recovered.len().min(8)]);
                        println!();
                    }
                    continue 'outer;
                }
...
fn run_kv(seed: u64, iterations: usize, ops_per: usize, max_report: usize,
          total: &mut usize, divergences: &mut usize) {
    use std::collections::HashMap;
    'outer: for iter in 0..iterations {
        let mut rng = Rng(seed.wrapping_add(0xABCD ^ iter as u64).wrapping_mul(0x100000001B3));
        ...
        for cycle in 0..cycles {
            let db = match open_dir(&tmp.0) {
                Ok(d) => d,
                Err(e) => { *divergences += 1; if *divergences <= max_report { println!("─── [kv] OPEN FAILED #{} (iter {iter}) ── {e}\n", *divergences); } continue 'outer; }
            };
                ...
                *total += 1;
                ...
                for (k, v) in model.iter() {
                    match kv.get(k) {
                        ...
                        other => { mismatch = Some(format!("key {k}: expected {v}, got {other:?}")); break; }
                    }
                }
                ...
                if mismatch.is_none() {
                    ...
                    if live.len() != model.len() {
                        mismatch = Some(format!("key count: expected {}, recovered {} ({:?})",
                            model.len(), live.len(),
                ...
                if let Some(m) = mismatch {
                    *divergences += 1;
                    if *divergences <= max_report {
                        println!("─── [kv] RECOVERY DIVERGENCE #{} (iter {iter}, cycle {cycle}, seed {seed}) ───", *divergences);
                ...
                match rng.below(5) {
                    ...
                    2 => {
                        ...
                        match kv.incr_by(&key, amt) {
                            Ok(newv) => { model.insert(key, newv); }
                            Err(_) => {}
                        }
                    }
                    3 => {
                        if kv.del(&key) { model.remove(&key); } else { model.remove(&key); }
                    }
                    _ => {
...
fn main_impl() {
    ...
    let r1 = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        run_disk_sql(seed, iterations, ops_per, max_report, &mut total, &mut divergences);
    }));
    if r1.is_err() {
        divergences += 1;
    ...
    let r2 = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        run_kv(seed, iterations, ops_per, max_report, &mut total, &mut divergences);
    }));
    if r2.is_err() {
        divergences += 1;
    ...
    println!("(LSM + Columnar durable engines are NOT wired into StorageMode — not testable via Database.)");
    if divergences == 0 {
