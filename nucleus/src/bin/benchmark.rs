...
fn kv_references() -> Vec<Reference> {
    vec![
        Reference {
            name: "Redis 7 (single-thread)",
            ops_per_sec: Some(100_000),
            latency_us: Some(10.0),
        },
        Reference {
            name: "SurrealDB 3 reads",
            ops_per_sec: Some(508_000),
            latency_us: None,
        },
        Reference {
            name: "SurrealDB 3 writes",
            ops_per_sec: Some(155_000),
            latency_us: None,
        },
    ]
}
...
fn sql_references() -> Vec<Reference> {
    vec![
        Reference {
            name: "PostgreSQL 17 writes",
            ops_per_sec: Some(205_000),
            latency_us: None,
        },
        Reference {
            name: "SurrealDB 3 writes",
...
fn bench_kv(db: &Database, scale: Scale) -> Vec<BenchResult> {
    ...
    // Mixed workload: 50% GET, 30% SET, 20% DEL
    {
        ...
        results.push(BenchResult {
            category: "KV".into(),
            workload: format!("Mixed 50R/30W/20D {mix_n}"),
            ops: mix_n,
            total_time: total,
            stats,
        });
...
async fn bench_sql(db: &Database, scale: Scale) -> Vec<BenchResult> {
    ...
    // Create tables
    db.execute(
        "CREATE TABLE bench_users (id BIGINT PRIMARY KEY, name TEXT NOT NULL, email TEXT, age INT)",
    )
    .await
    .unwrap();
    ...
    // Point query by PK
    {
        ...
        for i in 0..iters {
            let t = Instant::now();
            db.query(&format!("SELECT * FROM bench_users WHERE id = {i}"))
                .await
                .unwrap();
            stats.record(t.elapsed());
        }
        ...
        results.push(BenchResult {
            category: "SQL".into(),
            workload: format!("Point Query x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
    ...
    // Range scan
    {
        ...
        for i in 0..iters {
            let t = Instant::now();
            db.query(&format!(
                "SELECT * FROM bench_users WHERE id BETWEEN {} AND {}",
                i * 10,
                i * 10 + 99
            ))
            .await
        ...
        results.push(BenchResult {
            category: "SQL".into(),
            workload: format!("Range Scan 100 rows x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
    ...
    {
        let prep = db
            .prepare("INSERT INTO bench_users VALUES ($1, $2, $3, $4)")
            .unwrap();
        ...
        results.push(BenchResult {
            category: "SQL".into(),
            workload: format!("Prepared INSERT {prep_n} rows"),
            ops: prep_n,
            total_time: total,
            stats,
        });
    ...
    {
        let prep = db
            .prepare("SELECT * FROM bench_users WHERE id = $1")
            .unwrap();
        let iters = std::cmp::min(n, 1000);
        let start = Instant::now();
        ...
        for i in 0..iters {
            let t = Instant::now();
            db.query_prepared(&prep, &[Value::Int64(i as i64)])
                .await
                .unwrap();
            stats.record(t.elapsed());
        }
        ...
        results.push(BenchResult {
            category: "SQL".into(),
            workload: format!("Prepared Point Query x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
    ...
    // GROUP BY + SUM
    {
        ...
        for _ in 0..iters {
            let t = Instant::now();
            db.query("SELECT status, COUNT(*), SUM(amount) FROM bench_orders GROUP BY status")
                .await
                .unwrap();
            stats.record(t.elapsed());
        }
        ...
        results.push(BenchResult {
            category: "SQL".into(),
            workload: format!("GROUP BY+SUM x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
...
fn bench_fts(db: &Database, scale: Scale) -> Vec<BenchResult> {
    ...
    // BM25 search
    {
        ...
        results.push(BenchResult {
            category: "FTS".into(),
            workload: format!("BM25 Search x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
...
fn bench_graph(db: &Database, scale: Scale) -> Vec<BenchResult> {
    ...
    // Create nodes
    {
        ...
        for i in 0..nodes {
            ...
            props.insert(
                "name".to_string(),
                nucleus::graph::PropValue::Text(format!("node_{i}")),
            );
        ...
        results.push(BenchResult {
            category: "Graph".into(),
            workload: format!("Add {nodes} nodes"),
            ops: nodes,
            total_time: total,
            stats,
        });
    ...
    // Create edges
    {
        ...
        results.push(BenchResult {
            category: "Graph".into(),
            workload: format!("Add ~{total_edges} edges"),
            ops: total_edges,
            total_time: total,
            stats,
        });
    ...
    // BFS traversal
    {
        ...
        results.push(BenchResult {
            category: "Graph".into(),
            workload: format!("BFS 3-hop x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
    ...
    // Shortest path
    {
        ...
        results.push(BenchResult {
            category: "Graph".into(),
            workload: format!("Shortest Path x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
...
async fn bench_vector(db: &Database, scale: Scale) -> Vec<BenchResult> {
    ...
    // Insert vectors
    {
        ...
        results.push(BenchResult {
            ...
            workload: format!("Insert {n} x {dims}d"),
            ops: n,
            total_time: total,
            stats,
        });
    ...
    db.execute("CREATE INDEX bench_vecs_idx ON bench_vecs USING hnsw (embedding)")
        .await
        .unwrap();
    ...
    // Top-10 search via VECTOR_DISTANCE + ORDER BY + LIMIT
    {
        ...
        results.push(BenchResult {
            category: "Vector".into(),
            workload: format!("Top-10 HNSW x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
...
fn bench_timeseries(db: &Database, scale: Scale) -> Vec<BenchResult> {
    ...
    // Insert points
    {
        ...
        results.push(BenchResult {
            ...
            workload: format!("Insert {n} points"),
            ops: n,
            total_time: total,
            stats,
        });
    ...
    // Range aggregation
    {
        ...
        results.push(BenchResult {
            category: "TimeSeries".into(),
            workload: format!("Range SUM/COUNT/AVG x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
...
fn bench_document(db: &Database, scale: Scale) -> Vec<BenchResult> {
    ...
    // Insert documents
    {
        ...
        for i in 0..n {
            ...
            obj.insert(
                "name".to_string(),
                nucleus::document::JsonValue::Str(format!("user_{i}")),
            );
            obj.insert(
                "age".to_string(),
                nucleus::document::JsonValue::Number((20 + i % 50) as f64),
            );
            obj.insert(
                "city".to_string(),
                nucleus::document::JsonValue::Str(
    ...
    // Query by path
    {
        ...
        for i in 0..iters {
            ...
            let t = Instant::now();
            let _ = doc.query_by_path(
                &["city"],
                &nucleus::document::JsonValue::Str(city.to_string()),
            );
            stats.record(t.elapsed());
        }
        let total = start.elapsed();
        results.push(BenchResult {
            category: "Document".into(),
            workload: format!("Query by path x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
    ...
    // Get by ID
    {
        ...
        results.push(BenchResult {
            category: "Document".into(),
            workload: format!("Get by ID x{iters}"),
            ops: iters,
            total_time: total,
            stats,
        });
...
fn bench_columnar(db: &Database, scale: Scale) -> Vec<BenchResult> {
...

    {
        ...
        let total = start.elapsed();
        results.push(BenchResult {
            category: "Columnar".into(),
            workload: format!("Insert {n} rows (batched)"),
            ops: n,
            total_time: total,
            stats,
        });
...
fn print_comparison(results: &[BenchResult]) {
    ...
    if let Some(get) = kv_get {
        ...
        println!(
            "  KV GET: {} (Nucleus direct API, no network)",
            format_ops(nucleus_ops)
        );
    }

    // SQL comparisons
    let sql_insert = results
        .iter()
        .find(|r| r.workload.starts_with("INSERT") && r.category == "SQL");
        ...
        match args[i].as_str() {
            ...
            "--models" => {
                ...
                if let Some(models) = args.get(i) {
                    model_filter =
                        Some(models.split(',').map(|s| s.trim().to_lowercase()).collect());
                }
            }
            _ => {}
        }
