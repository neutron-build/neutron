...
async fn test_add_column_with_default() {
    ...
    exec(
        &ex,
        "ALTER TABLE df ADD COLUMN status TEXT DEFAULT 'active'",
    )
    .await;
...
async fn test_on_conflict_do_nothing() {
    ...
    exec(
        &ex,
        "INSERT INTO upsert1 VALUES (1, 'bob') ON CONFLICT (id) DO NOTHING",
    )
    .await;
...
async fn test_on_conflict_do_update() {
    ...
    exec(
        &ex,
        "INSERT INTO upsert2 VALUES (1, 'bob') ON CONFLICT (id) DO UPDATE SET name = 'bob'",
    )
    .await;
...
async fn test_pk_constraint_prevents_duplicate() {
    ...
    let result = ex
        .execute("INSERT INTO pk_test VALUES (1, 'charlie')")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
...
async fn test_unique_constraint_prevents_duplicate() {
    ...
    exec(
        &ex,
        "CREATE TABLE uq_test (id INT, email TEXT UNIQUE, name TEXT)",
    )
    .await;
    exec(
        &ex,
        "INSERT INTO uq_test VALUES (1, 'alice@test.com', 'Alice')",
    )
    .await;
    ...
    let result = ex
        .execute("INSERT INTO uq_test VALUES (3, 'alice@test.com', 'Charlie')")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("duplicate key") || err_msg.contains("unique constraint"));
...
async fn test_not_null_constraint_enforced() {
    ...
    exec(
        &ex,
        "CREATE TABLE nn_test (id INT NOT NULL, name TEXT NOT NULL, bio TEXT)",
    )
    .await;

    // NULL in NOT NULL column should fail
    let result = ex
        .execute("INSERT INTO nn_test VALUES (NULL, 'alice', 'hi')")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("not-null"));
    ...
    let result = ex
        .execute("INSERT INTO nn_test VALUES (1, NULL, 'hi')")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("not-null"));
...
async fn test_check_constraint() {
    ...
    exec(
        &ex,
        "CREATE TABLE ck_test (id INT, age INT CHECK (age >= 0), name TEXT)",
    )
    .await;

    // Value violating CHECK should fail
    let result = ex
        .execute("INSERT INTO ck_test VALUES (1, -5, 'alice')")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
...
async fn test_fk_constraint() {
    ...
    let result = ex
        .execute("INSERT INTO employees VALUES (3, 'Charlie', 999)")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
...
async fn test_constraint_on_update() {
    ...
    exec(
        &ex,
        "CREATE TABLE upd_nn (id INT NOT NULL, name TEXT NOT NULL)",
    )
    .await;
    exec(&ex, "INSERT INTO upd_nn VALUES (1, 'alice')").await;
    let result = ex
        .execute("UPDATE upd_nn SET name = NULL WHERE id = 1")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("not-null"));
    ...
    exec(
        &ex,
        "CREATE TABLE upd_uq (id INT PRIMARY KEY, email TEXT UNIQUE)",
    )
    .await;
    ...
    let result = ex
        .execute("UPDATE upd_uq SET email = 'a@b.com' WHERE id = 2")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("duplicate key") || err_msg.contains("unique constraint"));
    ...
    exec(
        &ex,
        "CREATE TABLE upd_parent (id INT PRIMARY KEY, name TEXT)",
    )
    .await;
    exec(&ex, "INSERT INTO upd_parent VALUES (1, 'dept1')").await;
    exec(
        &ex,
        "CREATE TABLE upd_child (id INT, parent_id INT REFERENCES upd_parent(id))",
    )
    .await;
    exec(&ex, "INSERT INTO upd_child VALUES (1, 1)").await;
    let result = ex
        .execute("UPDATE upd_child SET parent_id = 999 WHERE id = 1")
        .await;
    assert!(result.is_err());
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("foreign key"));
...
async fn test_insert_select_from_table() {
    ...
    exec(
        &ex,
        "INSERT INTO source_tbl VALUES (1, 'alice'), (2, 'bob'), (3, 'charlie')",
    )
    .await;
    exec(&ex, "CREATE TABLE target_tbl (id INT, name TEXT)").await;
    exec(
        &ex,
        "INSERT INTO target_tbl SELECT * FROM source_tbl WHERE id > 1",
    )
    .await;
...
async fn test_mergetree_boolean_default_false() {
    ...
    assert_eq!(
        r[0][1],
        Value::Bool(false),
        "BOOLEAN DEFAULT false should store as false, not true"
    );
}
...
async fn test_insert_mixed_default_and_literal() {
    ...
    exec(
        &ex,
        "CREATE TABLE def_tbl3 (id INT, val TEXT DEFAULT 'x', num INT DEFAULT 42)",
    )
    .await;
...
async fn test_on_conflict_excluded() {
    ...
    exec(
        &ex,
        "CREATE TABLE excl_tbl (id INT PRIMARY KEY, name TEXT, count INT)",
    )
    .await;
...
async fn test_on_conflict_excluded_expr() {
    ...
    exec(
        &ex,
        "CREATE TABLE excl_tbl2 (id INT PRIMARY KEY, counter INT)",
    )
    .await;
...
async fn test_fk_restrict_prevents_parent_delete() {
    ...
    exec(
        &ex,
        "CREATE TABLE fk_child (id INT, parent_id INT REFERENCES fk_parent(id) ON DELETE RESTRICT)",
    )
    .await;
    ...
    assert!(
        result.is_err(),
        "DELETE should fail with RESTRICT when children exist"
    );
    let err_msg = result.unwrap_err().to_string();
    assert!(
...
async fn test_fk_restrict_allows_delete_no_children() {
    ...
    exec(
        &ex,
        "CREATE TABLE fk_c2 (id INT, pid INT REFERENCES fk_p2(id) ON DELETE RESTRICT)",
    )
    .await;
...
async fn test_fk_cascade_delete() {
    ...
    exec(
        &ex,
        "INSERT INTO cascade_child VALUES (10, 1), (20, 1), (30, 2)",
    )
    .await;
    ...
    assert_eq!(
        *scalar(&r[0]),
        Value::Int64(1),
        "only child with pid=2 should remain"
    );
...
async fn test_fk_cascade_delete_multi_level() {
    ...
    exec(
        &ex,
        "CREATE TABLE ch (id INT, par_id INT REFERENCES par(id) ON DELETE CASCADE)",
    )
    .await;
    ...
    assert_eq!(
        *scalar(&r[0]),
        Value::Int64(0),
        "parent should be cascade-deleted"
    );
...
async fn test_fk_set_null_on_delete() {
    ...
    exec(
        &ex,
        "CREATE TABLE sn_parent (id INT PRIMARY KEY, name TEXT)",
    )
    .await;
    exec(&ex, "INSERT INTO sn_parent VALUES (1, 'X')").await;
    exec(
        &ex,
        "CREATE TABLE sn_child (id INT, pid INT REFERENCES sn_parent(id) ON DELETE SET NULL)",
    )
    .await;
    ...
    assert_eq!(
        rows(&r[0])[0][0],
        Value::Null,
        "child FK column should be NULL after SET NULL"
    );
}
...
async fn test_fk_set_default_on_delete() {
    ...
    assert_eq!(
        rows(&r[0])[0][0],
        Value::Int32(0),
        "child FK should be reset to default (0)"
    );
}
...
async fn test_fk_no_action_default() {
    ...
    exec(
        &ex,
        "CREATE TABLE na_child (id INT, pid INT REFERENCES na_parent(id))",
    )
    .await;
    ...
    assert!(
        result.is_err(),
        "default FK action (NoAction) should prevent delete with children"
    );
}
...
async fn test_fk_cascade_update() {
    ...
    exec(
        &ex,
        "CREATE TABLE cu_child (id INT, pid INT REFERENCES cu_parent(id) ON UPDATE CASCADE)",
    )
    .await;
    ...
    assert_eq!(
        rows(&r[0])[0][0],
        Value::Int32(99),
        "child FK should be updated to new parent PK"
    );
}
...
async fn test_fk_restrict_prevents_parent_update() {
    ...
    exec(
        &ex,
        "CREATE TABLE ru_child (id INT, pid INT REFERENCES ru_parent(id) ON UPDATE RESTRICT)",
    )
    .await;
    exec(&ex, "INSERT INTO ru_child VALUES (1, 1)").await;

    let result = ex
        .execute("UPDATE ru_parent SET id = 99 WHERE id = 1")
        .await;
    assert!(
        result.is_err(),
        "UPDATE should fail with RESTRICT when children reference old PK"
    );
    let err_msg = result.unwrap_err().to_string();
    assert!(
        err_msg.contains("foreign key"),
...
async fn test_fk_set_null_on_update() {
    ...
    exec(
        &ex,
        "CREATE TABLE su_child (id INT, pid INT REFERENCES su_parent(id) ON UPDATE SET NULL)",
    )
    .await;
    ...
    assert_eq!(
        rows(&r[0])[0][0],
        Value::Null,
        "child FK should be NULL after ON UPDATE SET NULL"
    );
}
...
async fn test_fk_multiple_children_cascade() {
    ...
    exec(
        &ex,
        "CREATE TABLE mc_child1 (id INT, pid INT REFERENCES mc_parent(id) ON DELETE CASCADE)",
    )
    .await;
    exec(
        &ex,
        "CREATE TABLE mc_child2 (id INT, pid INT REFERENCES mc_parent(id) ON DELETE CASCADE)",
    )
    .await;
...
async fn test_fk_partial_delete_cascade() {
    ...
    exec(
        &ex,
        "CREATE TABLE pd_child (id INT, pid INT REFERENCES pd_parent(id) ON DELETE CASCADE)",
    )
    .await;
...
async fn test_fk_constraint_parsing_on_delete_cascade() {
    ...
    let fk = table_def
        .constraints
        .iter()
        .find(|c| matches!(c, crate::catalog::TableConstraint::ForeignKey { .. }))
...
async fn test_fk_table_level_constraint_parsing() {
    ...
    assert_eq!(
        *scalar(&r[0]),
        Value::Int64(0),
        "table-level FK CASCADE should work"
    );
}
...
async fn test_fk_null_child_bypasses_constraint() {
    ...
    exec(
        &ex,
        "CREATE TABLE nb_child (id INT, pid INT REFERENCES nb_parent(id) ON DELETE RESTRICT)",
    )
    .await;
...
async fn test_fk_cascade_delete_all_parents() {
    ...
    exec(
        &ex,
        "CREATE TABLE da_child (id INT, pid INT REFERENCES da_parent(id) ON DELETE CASCADE)",
    )
    .await;
...
async fn test_fk_update_non_referenced_column_ok() {
    ...
    exec(
        &ex,
        "CREATE TABLE un_parent (id INT PRIMARY KEY, name TEXT)",
    )
    .await;
...
async fn test_fk_cascade_update_multi_level() {
    ...
    exec(
        &ex,
        "CREATE TABLE cu_ch (id INT, par_id INT REFERENCES cu_par(gp_id) ON UPDATE CASCADE)",
    )
    .await;
    ...
    assert_eq!(
        rows(&r[0])[0][0],
        Value::Int32(99),
        "parent FK should be 99"
    );
    let r = exec(&ex, "SELECT par_id FROM cu_ch").await;
    assert_eq!(
        rows(&r[0])[0][0],
        Value::Int32(99),
        "child FK should cascade to 99"
    );
}
...
async fn test_fk_set_default_on_update() {
    ...
    assert_eq!(
        rows(&r[0])[0][0],
        Value::Int32(0),
        "child FK should be reset to default (0) on update"
    );
}
...
async fn test_fk_mixed_actions_delete_cascade_update_restrict() {
    ...
    let result = ex
        .execute("UPDATE mx_parent SET id = 99 WHERE id = 1")
        .await;
    assert!(
        result.is_err(),
        "UPDATE should fail with ON UPDATE RESTRICT"
    );
...
async fn test_cte_insert_select() {
    ...
    exec(
        &ex,
        "WITH vals AS (SELECT 1 AS id, 'test' AS name) INSERT INTO cte_ins SELECT * FROM vals",
    )
    .await;
...
async fn test_cte_delete() {
    ...
    exec(
        &ex,
        "INSERT INTO cte_del VALUES (1, 'a'), (2, 'b'), (3, 'c')",
    )
    .await;
