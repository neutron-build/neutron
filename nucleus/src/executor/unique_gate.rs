...
impl super::Executor {
    ...
    pub(super) fn unique_slots_for_row(
        ...
        // `unique_col_sets` enumerates constraints first (declaration order),
        // then UNIQUE indexes, so its position doubles as a stable per-table
        // discriminator: a constraint keeps the slot id it always had, and an
        // index gets one past the constraint range that no constraint can
        // reach.
        let mut slots = Vec::new();
        for (cid, indices) in self
            .unique_col_sets(table_name, table_def)
            .into_iter()
            .enumerate()
        {
