...
impl Executor {
    ...
    /// faithfully recorded in the catalog (and round-tripped through
    /// catalog.json) but never consulted on the write path, so a
    /// `CREATE UNIQUE INDEX` on a populated table admitted duplicate keys
    /// forever. Uniqueness is a property of the key, not of how it was
    /// declared, so both spellings enumerate here.
    ///
    ...
    /// passed (or, on the gate path, occupy the same slot twice).
    pub(super) fn unique_col_sets(
        ...
        let mut push = |indices: Vec<usize>| {
            if !indices.is_empty() && !sets.contains(&indices) {
                sets.push(indices);
            }
        };
        ...
        // UNIQUE indexes: `IndexDef.unique` is the only place their uniqueness
        // is recorded. `get_indexes_cached` never fails for a table that
        // exists, and an empty list is the correct answer for one with no
        // indexes, so the `None` case needs no special treatment.
        for index in self
            .catalog
